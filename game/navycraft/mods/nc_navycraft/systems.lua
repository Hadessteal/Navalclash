...
local function default_systems(profile,owner)
    ...
    return {
        ...
        engines=engines,engines_on=false,top_speed=0,
        ...
        flooding=0,hull_integrity=1,helm_destroyed=false,sinking=false,scuttle_at=0,
...
local function update_damage_and_pumps(construct,dt)
    local p,s=construct.profile,construct.systems
    local pump_count=p.pump_count or 0
    if s.pump_on and pump_count>0 and s.flooding>0 and s.pump_charge>0 then
        local equipment=s.equipment_modifiers or{}
    ...
    if s.helm_destroyed or s.hull_integrity<.28 then s.sinking=true end
    if s.sinking then
        s.throttle=0;s.gear=0;s.hyperdrive=false
        s.flooding=s.flooding+math.max(.1,(1-s.hull_integrity))*dt
    end
end
...
function S.step(construct,dt)
    ...
    local speed=top_speed*throttle*gear_ratio*direction
    ...
    elseif craft_type.can_navigate and s.buoyancy<0 then vertical=-.5
...
function S.damage_hook(construct,removed,position,attacker,damage_type)
    local s=construct.systems
    if not s then return end
    if attacker and attacker~="" then s.damagers[attacker]=(s.damagers[attacker] or 0)+removed end
    if attacker and attacker~="" and navycraft.campaign and navycraft.campaign.missions then
        navycraft.campaign.missions.record_damage(attacker,construct,removed)
    end
    if removed>=math.max(3,construct.profile.block_count*.1) then local flood_mult=((s.equipment_modifiers or{}).flooding_mult or 1);s.flooding=s.flooding+removed*flood_mult end
    ...
    if navycraft.effects then navycraft.effects.damage(construct,position,removed) end
end
...
function S.toggle_engine(c,node_index) local state=c.systems.engines[tostring(node_index)];if not state then return nil end;state.set_on=not state.set_on;return state.set_on end
function S.cycle_sonar(c) local i=1;for n,v in ipairs(sonar_names) do if v==c.systems.sonar_mode then i=n end end;i=i%#sonar_names+1;c.systems.sonar_mode=sonar_names[i];return c.systems.sonar_mode end
function S.set_pump(c,on) c.systems.pump_on=on end
...
function S.summary(c)
    ...
    return string.format("%s/%s%s | blocks=%d/%d health=%d%% mass=%.1f cargo=%.1f/%.1f disp=%.1f buoy=%d flood=%.1f maint=%d%% | engines=%d/%d top=%.1f throttle=%d%% gear=%d rudder=%.1f | ballast=%s %.0f%% pumps=%s %.1f | sub=%s radar=%s sonar=%s launcher=%s route=%s:%d",
        ...
        s.operating_mass or p.weight,s.cargo_used or 0,s.cargo_capacity or 0,s.displacement,s.buoyancy,s.flooding,condition,running,total,s.top_speed or 0,math.floor(s.throttle*100+.5),s.gear,s.rudder,
